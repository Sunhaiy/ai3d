...
在 RTX 4060 系列显卡上从随机权重训练一个小型“单张图片生成 3D”模型。网页默认使用 `128 x 128` 输入图片和 `32 x 32 x 32` 体素，并通过 marching cubes 导出 Blender 可以打开的平滑 `.obj`。
...
- `256³`：16GB 显存长时训练档，使用可扩展的逐级 3D 解码器和梯度检查点，默认批大小 1、1000 轮并在累计训练约 72 小时后正常结束。8GB 显存不建议使用此档。
- `512³`：隐式表面档。训练时抽取空间占用点，生成时仅细分物体表面附近的网格块，默认批大小 1、1000 轮、72 小时。
- `1024³`：最高隐式表面档，使用相同的分块方案，不会在显存中创建完整的 `1024³` 稠密张量；生成耗时会明显长于 `512³`。

训练数据按“多张图片索引到一个唯一体素目标”保存，不会再为同一个模型的 56 张图片重复存储 56 份高分辨率体素。
`256³` 的体素数是 `128³` 的 8 倍、`32³` 的 512 倍。以 214 个唯一模型计算，仅未压缩的 `uint8` 体素目标约占 3.6GB 内存，制作数据时还需要为图片、网格处理和临时数组预留额外内存。
直接保存 214 个 `1024³` 稠密标签约需 214GB，因此 `512³` 和 `1024³` 会改存每个模型 65,536 个均匀/近表面采样点。高分辨率采样依赖封闭网格；严重破面、无法判断内外的模型会在制作数据日志中标记为 `SKIP`。隐式检查点生成的 `result.npy` 是用于定位表面的粗采样体素，完整分辨率体现在自适应生成的 OBJ 坐标中。
...
python train_image_to_3d.py `
  ...
  --batch-size 1 `
  --max-hours 72 `
  --output runs/image_to_3d_256.pt
```
